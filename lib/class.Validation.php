...
class Validation
{
         ...
         * 戻値：        ：正規の値ならば真を、問題があれば偽を返す
         ****************************************************************************/
        function isDW_Book($value = "", $require = false, $max = 14, $min = 0, $enc_chk = "UTF-8", $enc_sou = "UTF-8"){
                $ret = true;
                if (!$this->isMatch($value, $require, "/^(".VALIDATION_ISBN."|".VALIDATION_ASIN.")+$/", $max, $min, $enc_chk, $enc_sou))
                {
                        $ret = false;
                }
                return $ret;
