...
define("VALIDATION_ASIN",    "/^[A-Z0-9]{10}$/"); //半角大文字英数字
...
class Validation
{
        ...
        {
                $ret = true;
                if (!$this->isMatch($value, $require, "/^(".VALIDATION_SW_LLETTER."OR".ALIDATION_SW_DIGIT.")+$/", $max, $min, $enc_chk, $enc_sou))
                {
                        $ret = false;
                }
                return $ret;
         ...
         * 戻値：        ：正規の値ならば真を、問題があれば偽を返す
         ****************************************************************************/
        function isISBN($value = "", $require = false, $max = 14, $min = 0, $enc_chk = "UTF-8", $enc_sou = "UTF-8"){
                $ret = true;
         ...
         * 戻値：        ：正規の値ならば真を、問題があれば偽を返す
         ****************************************************************************/
        function isASIN($value = "", $require = false, $max = 10, $min = 0, $enc_chk = "UTF-8", $enc_sou = "UTF-8"){
                $ret = true;
                if (!$this->isMatch($value, $require, VALIDATION_ASIN, $max, $min, $enc_chk, $enc_sou))
                {
                        $ret = false;
                }
                return $ret;
